defmodule LedgerBankApi.Users.ContextAuthTest do
  ...
  test "revoking all tokens logs out all sessions", %{user: user} do
    ...
    assert {:ok, _db_user, _access1, refresh1} = Context.login_user(user.email, "abc12345")
    assert {:ok, _db_user2, _access2, refresh2} = Context.login_user(user.email, "abc12345")
    # Revoke all tokens
