defmodule LedgerBankApi.Auth.JWTTest do
  ...
  describe "JWT token generation and verification" do
    test "generates and verifies an access token with role and email claims", %{user: user} do
      {:ok, token} = Auth.generate_access_token(user)
    ...
    test "generates and verifies a refresh token", %{user: user} do
      {:ok, token} = Auth.generate_refresh_token(user)
      assert {:ok, claims} = Auth.verify_token(token)
      assert claims["sub"] == user.id
      assert claims["role"] == user.role
      assert claims["email"] == user.email
    ...
    test "generates tokens with different user roles", %{admin_user: admin_user} do
      {:ok, token} = Auth.generate_access_token(admin_user)
      assert {:ok, claims} = Auth.verify_token(token)
  ...
  describe "JWT token validation" do
    test "rejects expired tokens", %{user: user} do
      ...
      assert false == Auth.token_expired?(token)
    ...
    test "rejects invalid signature", %{user: user} do
      ...
      assert {:error, :signature_error} = Auth.verify_token(tampered_token)
    end

    test "rejects malformed tokens" do
      # The actual error is :signature_error for malformed tokens
      assert {:error, :signature_error} = Auth.verify_token("invalid.token.here")
    end
    ...
    test "rejects non-string tokens" do
      assert {:error, :invalid_token} = Auth.verify_token(123)
      assert {:error, :invalid_token} = Auth.verify_token(%{})
      assert {:error, :invalid_token} = Auth.verify_token([])
    end
  ...
  describe "JWT token refresh" do
    test "refreshes access token using valid refresh token", %{user: user} do
      {:ok, refresh_token} = Auth.generate_refresh_token(user)
      assert {:ok, new_access_token, new_refresh_token} = Auth.refresh_access_token(refresh_token)
    ...
    test "rejects refresh with access token", %{user: user} do
      {:ok, access_token} = Auth.generate_access_token(user)
      assert {:error, :invalid_refresh_token} = Auth.refresh_access_token(access_token)
    end


    test "rejects refresh with invalid refresh token", %{user: _user} do
      assert {:error, :invalid_refresh_token} = Auth.refresh_access_token("invalid.token")
    end
  end
  ...
  describe "JWT utility functions" do
    test "gets user_id from valid token", %{user: user} do
      {:ok, token} = Auth.generate_access_token(user)
      assert {:ok, user_id} = Auth.get_user_id(token)
    ...
    test "get_user_id fails with invalid token", %{user: _user} do
      assert {:error, :invalid_token} = Auth.get_user_id("invalid.token")
      assert {:error, :invalid_token} = Auth.get_user_id(nil)
    end
  ...
  describe "JWT security and production scenarios" do
    test "tokens have unique jti for refresh tokens", %{user: user} do
      {:ok, refresh_token1} = Auth.generate_refresh_token(user)
      {:ok, refresh_token2} = Auth.generate_refresh_token(user)

      assert {:ok, claims1} = Auth.verify_token(refresh_token1)
      assert {:ok, claims2} = Auth.verify_token(refresh_token2)
  ...
  describe "JWT edge cases and error handling" do
    ...
    test "handles user IDs and emails correctly", %{user: user} do
      {:ok, token} = Auth.generate_access_token(user)
      assert {:ok, claims} = Auth.verify_token(token)
      assert claims["sub"] == user.id
      assert claims["email"] == user.email
