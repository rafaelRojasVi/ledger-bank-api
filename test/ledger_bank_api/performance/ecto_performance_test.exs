defmodule LedgerBankApi.Performance.EctoPerformanceTest do
  ...
  describe "Pagination Performance" do
    test "paginating through 1000 users performs consistently" do
      ...
      # ========================================================================
      page_times =
        Enum.map(1..10, fn page_num ->
          start_time = System.monotonic_time(:microsecond)

          _users = UserService.list_users(pagination: %{page: page_num, page_size: 100})
    ...
    test "keyset pagination outperforms offset pagination for large datasets" do
      ...
      _offset_users =
        UserService.list_users(
          # Page 25 = offset 480
          pagination: %{page: 25, page_size: 20}
        )
      ...
      _final_result =
        Enum.reduce_while(1..24, first_page, fn _page, current_result ->
  ...
  describe "Query Optimization - Complex Queries" do
    test "filtering and sorting 500 users performs well" do
      ...
      IO.puts(
        "   Simple filter: #{Float.round(filter_duration / 1000, 2)}ms (found #{length(admin_users)})"
      )

      IO.puts(
        "   Filter + Sort: #{Float.round(filter_sort_duration / 1000, 2)}ms (found #{length(sorted_users)})"
      )

      IO.puts(
        "   Complex query: #{Float.round(complex_duration / 1000, 2)}ms (found #{length(complex_users)})"
      )
    ...
    test "searching users by email prefix is indexed" do
      ...
      query =
        from(u in LedgerBankApi.Accounts.Schemas.User,
  ...
  describe "Payment Query Performance" do
    test "listing payments with filters scales linearly" do
      ...
      results =
        Enum.map(test_cases, fn {filters, description} ->
          # Count queries
          {query_count, _} =
            count_queries_with_result(fn ->
              {payments, _} =
                FinancialService.list_user_payments(
                  user.id,
                  filters: filters,
      ...
      Enum.each(results, fn {desc, count, duration, queries} ->
        IO.puts(
          "   #{desc}: #{Float.round(duration / 1000, 2)}ms (#{count} results, #{queries} queries)"
        )
      end)
  ...
  describe "Large Dataset Query Performance" do
    test "fetching user statistics is O(1) not O(n)" do
      ...
      # ========================================================================
      [
        {_size1, duration1, queries1},
        {_size2, duration2, queries2},
        {_size3, duration3, queries3}
      ] = results
  ...
  describe "Transaction and Isolation Performance" do
    ...
    test "concurrent payment processing maintains balance integrity" do
      ...
      failure_reasons =
        Enum.map(failure_results, fn {_id, {:error, error}} ->
          error.reason
        end)
        |> Enum.frequencies()
      ...
      actual_payments_processed =
        Decimal.to_integer(Decimal.div(actual_deducted, Decimal.new("10")))
      ...
      if actual_payments_processed < success_count do
        IO.puts(
          "   ⚠️  DETECTED: #{success_count - actual_payments_processed} payments succeeded in API but didn't update balance"
        )
    ...
    test "diagnose concurrent processing bottleneck with detailed logging" do
      ...
      # Analyze results
      success_results =
        Enum.filter(results, fn {_id, result, _dur} -> match?({:ok, _}, result) end)

      failure_results =
        Enum.filter(results, fn {_id, result, _dur} -> match?({:error, _}, result) end)
      ...
      failure_breakdown =
        if length(failure_results) > 0 do
          breakdown =
            Enum.map(failure_results, fn {_id, {:error, error}, _dur} ->
              error.reason
            end)
            |> Enum.frequencies()
      ...
      IO.puts(
        "     Payments processed: #{payments_processed} out of #{length(success_results)} successful"
      )
    ...
    test "verify duplicate detection vs race condition - different amounts" do
      ...
      cond do
        success_count == 10 and Decimal.eq?(actual_deducted, expected_total) ->
          ...
          IO.puts(
            "     ✅ Previous test failure was due to duplicate detection (correct behavior)"
          )
  ...
  # End of Transaction and Isolation Performance
